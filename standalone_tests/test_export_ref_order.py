...
for line in open("/home/nick/prog/sb/export-stuff/salex_digi_250825.jsonl", "r"):
    ...
    saol = entry.get("saol")
    if not saol: continue
    if saol.get("endastDigitalt", False): continue
    ...
    for hb in saol.get("huvudbetydelser", []):
        ...
        for hv in hvs:
            ...
            refs.append(Ref(hv["typ"], id, hv_info["ortografi"], hv_info["homografNr"], hv_info.get("huvudbetydelseNr")))

        # check for assyriska problem
        for ref1, ref2 in zip(refs, refs[1:]):
            same_types = ref1.typ == ref2.typ or ref2.typ in [",", "och"]
            if same_types and ref1.homografNr == ref2.homografNr and ref1.ortografi == ref2.ortografi and ref1.huvudbetydelseNr >= ref2.huvudbetydelseNr:
                wrong_order.append(entry)
            elif same_types and ref1.ortografi == ref2.ortografi and ref1.huvudbetydelseNr == ref2.huvudbetydelseNr and ref1.homografNr >= ref2.homografNr:
                wrong_order.append(entry)
...
for types, entries in sorted(groups.items(), key = lambda x: x[1][0][0]["ortografi"]):
    ...
    for entry, refs in entries:
        ...
        print(" / ".join(ref_strs))
        if types not in interest_groups: break
