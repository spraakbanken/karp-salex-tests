...
from utils.salex import visible_part, entry_is_visible_in_printed_book
...
#    entry = visible_part(json.loads(line), test=entry_is_visible_in_printed_book)
    ...
    if entry["ingångstyp"] == "variant": continue

    abstracted = abstract(entry)
    groups[deepfreeze(abstracted)].append(entry)

entries_of_interest = {"akne", "vanilj", "rullad", "kex", "riksha",
                       "rakit", "stopplikt", "rhododendron",
                       "kollektiv", "gigawattimme", "riksha"}
